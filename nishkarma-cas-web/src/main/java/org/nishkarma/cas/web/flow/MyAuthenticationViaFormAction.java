package org.nishkarma.cas.web.flow;
...
public class MyAuthenticationViaFormAction {
	 ...
	 * matches the login ticket provided by the request. The comparison is
	 * case-sensitive.
	 *
	 * @param context
	 *            the context
	...
	protected boolean checkLoginTicketIfExists(final RequestContext context) {
		final String loginTicketFromFlowScope = WebUtils
				.getLoginTicketFromFlowScope(context);
		final String loginTicketFromRequest = WebUtils
		...
		return StringUtils.equals(loginTicketFromFlowScope,
	...
	protected Event returnInvalidLoginTicketEvent(final RequestContext context,
		...
		messageContext.addMessage(new MessageBuilder().error()
				.code("error.invalid.loginticket").build());
	 ...
	 * @since 4.1.0
	 */
	protected boolean isRequestAskingForServiceTicket(
			final RequestContext context) {
		final String ticketGrantingTicketId = WebUtils
				.getTicketGrantingTicketId(context);
		final Service service = WebUtils.getService(context);
		return (StringUtils.isNotBlank(context.getRequestParameters().get(
	 ...
	 * @since 4.1.0
	 */
	protected boolean addWarningMessagesToMessageContextIfNeeded(
			final TicketGrantingTicket tgtId,
			final MessageContext messageContext) {
		boolean foundAndAddedWarnings = false;
		for (final Map.Entry<String, HandlerResult> entry : tgtId
				.getAuthentication().getSuccesses().entrySet()) {
			for (final MessageDescriptor message : entry.getValue()
					.getWarnings()) {
	 ...
	 *            the context
	 */
	private void putPublicWorkstationToFlowIfRequestParameterPresent(
			final RequestContext context) {
		/*
		 * if (StringUtils.isNotBlank(context.getExternalContext()
		 * .getRequestParameterMap().get(PUBLIC_WORKSTATION_ATTRIBUTE))) {
		 * 
		 * context.getFlowScope().put(PUBLIC_WORKSTATION_ATTRIBUTE,
		 * Boolean.TRUE); }
		 */

		if (MyClientInfoHolder.getClientInfo().isProtectPublicWorkstation()) {
			context.getFlowScope().put(PUBLIC_WORKSTATION_ATTRIBUTE,
					Boolean.TRUE);
		}
	 ...
	 *            the ticket registry. No longer needed as the core service
	 *            layer returns the correct object type. Will be removed in
	 *            future versions.
	 ...
	 *            Warning message.
	 */
	private void addWarningToContext(final MessageContext context,
			final MessageDescriptor warning) {
		final MessageBuilder builder = new MessageBuilder().warning()
