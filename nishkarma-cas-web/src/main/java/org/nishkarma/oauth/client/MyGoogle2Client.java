...
import org.nishkarma.oauth.builder.api.MyGoogleApi20;
import org.nishkarma.oauth.profile.MyGoogle2AttributesDefinition;
import org.nishkarma.oauth.profile.MyGoogle2Profile;
 ...
 * @since 1.2.0
 */
public class MyGoogle2Client extends BaseOAuth20Client<MyGoogle2Profile> {
    ...
    public MyGoogle2Client(final String key, final String secret) {
        setKey(key);
        setSecret(secret);
        logger.debug("---MyGoogle2Client init");
    }
	...
	protected void internalInit() {
        ...
        this.service = new ProxyOAuth20ServiceImpl(new MyGoogleApi20(), new OAuthConfig(this.key, this.secret,
                this.callbackUrl,
    ...
    protected MyGoogle2Profile extractUserProfile(final String body) {
        ...
        if (json != null) {
            ...
            profile.addAttribute(MyGoogle2AttributesDefinition.DISPLAY_NAME, JsonHelper.get(json, MyGoogle2AttributesDefinition.DISPLAY_NAME));
